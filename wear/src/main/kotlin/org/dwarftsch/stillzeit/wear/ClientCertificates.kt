...
/**
 * Baut aus den vom Telefon übernommenen PEM-Dateien (client.crt / client.key)
 * eine SSLSocketFactory, die das Client-Zertifikat beim TLS-Handshake anbietet.
 *
 * Unterstützt werden dieselben Dateien wie auf dem Telefon: PEM ohne
 * Passphrase, Schlüssel als PKCS#8 (`BEGIN PRIVATE KEY`) oder PKCS#1
 * (`BEGIN RSA PRIVATE KEY`).
 */
object ClientCertificates {

    fun socketFactory(certPem: ByteArray, keyPem: ByteArray): SSLSocketFactory {
        ...
        // trustManagers = null → System-Trust-Store, das Server-Zertifikat wird
        // also weiterhin normal geprüft (wie in api_service.dart).
        val context = SSLContext.getInstance("TLS")
        context.init(keyManager.keyManagers, null, null)
